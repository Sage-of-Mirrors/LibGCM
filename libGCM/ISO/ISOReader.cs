...
namespace libGCM.ISOs
{
    public partial class ISO
    {
        ...
        private VirtualFilesystemFile GetFileData(FSTEntry fstData, EndianBinaryReader image)
        {
            ...
            {
                file = new VirtualFilesystemFile(fileNameAndExtension[0], "", new VirtualFileContents(data));
            }

            else
            {
                file = new VirtualFilesystemFile(fileNameAndExtension[0], "." + fileNameAndExtension[1], new VirtualFileContents(data));
            }
